...
function emotionalIntelligenceRecipes(player)
    ...
    if player:getModData().DTangerIntelligence == false and playerRecipes:contains("Anger management volume 1") and 
    ...
    playerRecipes:contains("Anger management volume 4") and playerRecipes:contains("Anger management volume 5") then
        player:getModData().DTangerIntelligence = true;
    end
end
...
function bloodlustTraitEffects(player)
    ...
    player:getModData().DTboredomIntelligence == true and player:getModData().DTangerIntelligence == true then
        if player:getModData().DTtimesinceLastKill > 72 then
            ...
            -- STRESS
            DTincreaseStress(player, ZombRand(5), 0.30);
            -- BOREDOM
            DTincreaseBoredom(player, ZombRand(5), 25);
            -- UNHAPPYNESS
            DTincreaseUnhappyness(player, ZombRand(5), 20);
            -- ANGER
            DTincreaseAnger(player, ZombRand(5), 0.45);
        -- IF PLAYER HAVEN'T KILLED ZOMBIES FOR 72 HOURS, THEN THE MOOD WILL BE AFFECTED EVERY HOUR
        elseif player:getModData().DTtimesinceLastKill > 48 then
        ...
        if player:getModData().DTtimesinceLastKill > 48 then
        ...
        elseif player:getModData().DTtimesinceLastKill > 24 then
...
--[[function bloodlustTraitSounds(player)
    ...
    if not player:isAsleep() then
        ...
        if player:getModData().DTstressIntelligence == true and player:getModData().DTunhappynessIntelligence == true and
        player:getModData().DTboredomIntelligence == true and player:getModData().DTangerIntelligence == true then
            -- IF PLAYER HAVEN'T KILLED ZOMBIES FOR 120 HOURS, THEN THE PLAYER WILL START TO HEAR RANDOM SOUNDS
            if player:getModData().DTtimesinceLastKill > 120 then
                DTPlaySound(player, ZombRand(180), randomSoundToPlay[ZombRand(1,#randomSoundToPlay)]);
            end
        else
            -- IF PLAYER HAVEN'T KILLED ZOMBIES FOR 48 HOURS, THEN THE PLAYER WILL START TO HEAR RANDOM SOUNDS
            if player:getModData().DTtimesinceLastKill > 48 then
                DTPlaySound(player, ZombRand(180), randomSoundToPlay[ZombRand(1,#randomSoundToPlay)]);
            end
        end
