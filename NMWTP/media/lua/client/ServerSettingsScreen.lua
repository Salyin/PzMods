...
function ServerSettingsScreenModsPanel:createChildren()
    ...
    self.warningText = ISLabel:new(self.listbox:getX(), comboBox:getBottom() + 8, fontHgt, getText(getText("UI_no_more_workshop_tab_problem_welcome")), 1, 1, 1, 1, UIFont.Medium, true)
    self:addChild(self.warningText)

    label = ISLabel:new(self.listbox:getX(), self.warningText:getBottom() + 12, 20, getText("UI_ServerSettings_AddOtherMod"), 1, 1, 1, 1, UIFont.Medium, true)
    self:addChild(label)
...
function ServerSettingsScreenModsPanel:setSettings(settings)
    ...
    self.warningText:setName(getText("UI_no_more_workshop_tab_problem_welcome"))
    self.warningText:setColor(0,1,0)
end
...
function ServerSettingsScreenModsPanel:addModToList(modID)
    -- print("ServerSettingsScreenModsPanel:addModToList")
    local item = {}
    item.modID = modID
    ...
    if item.modInfo then
        ...
        if modInfo:getRequire() and not modInfo:getRequire():isEmpty() then
            local description = getText("UI_no_more_workshop_tab_problem_require")
            local err = false
            for i=1,modInfo:getRequire():size() do
                local modID2 = modInfo:getRequire():get(i-1)
                local modInfo1 = getModInfoByID(modID2)
                if modInfo1 == nil then
                    description = description .. " " .. modID2
                    err = true
                elseif not self:findModInList(modID2) then
                    description = description .. " " .. modInfo1:getName()
                    err = true
                end
            end
            if err then
                self.warningText:setName(description)
                self.warningText:setColor(1,0,0)
                return
            end
        end
        if item.modInfo:getWorkshopID() then
            if not NoMoreWorkshopProblemTable[item.modInfo:getWorkshopID()] then
                self.warningText:setName(getText("UI_no_more_workshop_tab_problem_forgot"))
                self.warningText:setColor(1,0,0)
            else
                self.listbox:addItem(item.modInfo:getName(), item)
                self.warningText:setName(getText("UI_no_more_workshop_tab_problem_ok"))
                self.warningText:setColor(0,1,0)
            end
        else
            self.listbox:addItem(item.modInfo:getName(), item)
            self.warningText:setName(getText("UI_no_more_workshop_tab_problem_local"))
            self.warningText:setColor(1,1,0)
        end
    else
        ...
        if modInfo then
            ...
            if modInfo:getWorkshopID() then
                if not NoMoreWorkshopProblemTable[modInfo:getWorkshopID()] then
                    self.warningText:setName(getText("UI_no_more_workshop_tab_problem_forgot"))
                    self.warningText:setColor(1,0,0)
                else
...
function ServerSettingsScreenWorkshopPanel:createChildren()
    -- print("ServerSettingsScreenWorkshopPanel:createChildren")
    local fontHgt = getTextManager():getFontFromEnum(UIFont.Medium):getLineHeight()
...
NoMoreWorkshopProblemTable = {}

function ServerSettingsScreenWorkshopPanel:setSettings(settings)
    -- print("ServerSettingsScreenWorkshopPanel:setSettings")
    self.settings = settings
    self.listbox:clear()
    NoMoreWorkshopProblemTable = {}
    ...
    for _,workshopID in ipairs(workshopIDs) do
        if workshopID ~= "" then
            ...
            NoMoreWorkshopProblemTable[workshopID] = true
        end
    end
end
...
function ServerSettingsScreenWorkshopPanel:onRemoveItem(index)
    ...
    self.listbox:removeItemByIndex(index)
    NoMoreWorkshopProblemTable[workshopID] = nil
