...
function ShGO.createServerGlobalObjectType(key,funcGetSpriteName)
    ...
    function oType:new(luaSystem, globalObject)
        ...
        if ShGO.Verbose then print('SGOs['..key..']:new '..ShGO.getTableString(o)) end
        return o
    end
    ...
    function oType:fromModData(modData)
        if ShGO.Verbose then print('SGOs['..key..']:fromModData '..ShGO.getTableString(modData[key] or 'nil')) end
        self[key] = modData[key]
    end

    function oType:toModData(modData)
        if ShGO.Verbose then print('SGOs['..key..']:toModData '..ShGO.getTableString(self[key] or 'nil')) end
...
function ShGO.createServerGlobalObjectSystemType(key)
    ...
    function oSysType:initSystem()
        ...
        self.system:setObjectModDataKeys({'transfer'})
    end
        ...
        for i=1,globalObjects:size() do
            ...
            if square then--hum why would we be watching the chunk object if the square is not ready ?
                ...
                if isoObject then
                    ...
                    if ShGO.Verbose then print('OnChunkLoaded '..i..' '..ShGO.getTableString(globalObject:getModData())..' '..ShGO.getTableString(isoObject:getModData())) end
    ...
    function oSysType:loadIsoObject(isoObject)
        if ShGO.Verbose then print('SGOSystems['..key..']:loadIsoObject 1 '..ShGO.getTableString(isoObject:getModData())) end
        SGlobalObjectSystem.loadIsoObject(self,isoObject)
        if ShGO.Verbose then print('SGOSystems['..key..']:loadIsoObject 2 '..ShGO.getTableString(isoObject:getModData())) end
    end
    ...
    function oSysType:newLuaObject(globalObject)
        if ShGO.Verbose then print('SGOSystems['..key..']:newLuaObject '..ShGO.getTableString(globalObject)) end
        local luaObj = SGOs[key]:new(self, globalObject)
        if luaObj then
            if ShGO.Verbose then print('SGOSystems['..key..']:newLuaObject 2 '..ShGO.getTableString(luaObj)) end
        else
    ...
    function oSysType:isValidIsoObject(isoObject)
        ...
        if ShGO.Verbose then print('SGOSystems['..key..']:isValidIsoObject '..tostring(valid and 'true  ' or 'false ')..tostring(isoObject or 'nil')..' '..ShGO.getTableString(isoObject and isoObject:getModData()[key] or 'nil')) end
    ...
    oSysType['add_'..key] = function(self,playerObj,args)
        if ShGO.Verbose then print('SGOSystems['..key..']:add_'..key..' '..ShGO.getTableString(args)) end
        ...
        luaObject:initNew()
        luaObject[key].transfer = ShGO.overrideCGOArgsWithParams(luaObject[key].transfer,args.transfer)
        if ShGO.Verbose then print('SGOSystems['..key..']:add_'..key..' 2 '..ShGO.getTableString(args)..' '..ShGO.getTableString(luaObject[key].transfer)) end
        luaObject:addObject()
    ...
    oSysType['remove_'..key] = function(self,playerObj,args)
        if ShGO.Verbose then print('SGOSystems['..key..']:remove_'..key..' '..ShGO.getTableString(args)) end
    ...
    oSysType['update_'..key] = function(self,playerObj,args)
        if ShGO.Verbose then print('SGOSystems['..key..']:update_'..key..' '..ShGO.getTableString(args)) end
        local luaObject = self:getLuaObjectAt(args.x, args.y, args.z)
        if not luaObject then print('SGOSystems['..key..']:update_'..key..' failed no luaobject.'); return false end
        self[key] = ShGO.overrideCGOArgsWithParams(self[key],args)
        self:noise('updated')
        luaObject:saveData()
        return luaObject
    end
    ...
    function oSysType:OnClientCommand(command, playerObj, args)
        if ShGO.Verbose then print('SGOSystems['..key..']:OnClientCommand '..tostring(command or 'nil')..' '..ShGO.getTableString(args)) end
    ...
    oSysType['MaybeNew_'..key]=function(isoObject)
        ...
        if ShGO.Verbose then print('SGOSystems['..key..']:MaybeNew => from pre-existing isoObject '..tostring(valid and 'true ' or 'false ')..tostring(isoObject or 'nil')..' '..ShGO.getTableString(isoObject and isoObject:getModData() or 'nil')) end
        if isoObject and isoObject:getSquare() then
            local sq = isoObject:getSquare()
            local luaObject = self:getLuaObjectOnSquare(sq)
            if ShGO.Verbose then print('SGOSystems['..key..']:MaybeNew 2> from pre-existing square ['..sq:getX()..','..sq:getY()..','..sq:getZ()..']'..ShGO.getTableString(luaObject)) end
        end
    end
    ...
    oSysType['MaybeLoad_'..key]=function(isoObject)
        if ShGO.isValidIsoObj(isoObject,key) then
            SGOSystems[key].instance:loadIsoObject(isoObject)
            if ShGO.Verbose then print('SGOSystems['..key..']:MaybeLoad => loadIsoObject from pre-existing isoObject '..tostring(isoObject)..' '..ShGO.getTableString(isoObject:getModData())) end
        end
    end
