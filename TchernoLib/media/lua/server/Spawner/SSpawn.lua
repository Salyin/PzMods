...
function Spawn.removeFromSpawn(rKey,inhibitFromModData)
    for iter,val in pairs(Spawn.spawnDefinitions) do
...
function Spawn.sameSquarePos(square,pos)
    return square:getX() == pos.x and square:getY() == pos.y and square:getZ() == pos.z
end
...
function Spawn.spawnOnLoadGridsquare(square)
    for it,struct in pairs(Spawn.spawnDefinitions) do
        if Spawn.sameSquarePos(square,struct) then
...
function Spawn.spawnOnInitGMD()
    ...
    if md then
        local toRemove = {}
        for key,val in pairs(Spawn.spawnDefinitions) do
