...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.Trumpet' then -- CHANGE THIS LINE
				
				local PlayTrumpetTracks = require("TimedActions/PlayTrumpetTracks")
				...
				for k,v in pairs(PlayTrumpetTracks) do
					if level > 3 then
				...
				if level < 4 then
				actionType = "Bob_PlayTrumpetDefaultBeginner"
				else
				actionType = "Bob_PlayTrumpetDefault"
				end
			...
			if not isSitOnGround then
				...
				if randomTrack.level >= 8 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayTrumpetProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayTrumpetProficientB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayTrumpetProficientC"
					end
				...
				elseif randomTrack.level >= 4 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayTrumpetProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayTrumpetExperiencedC"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayTrumpetExperiencedC"
					end
				...
				elseif randomTrack.level >= 2 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayTrumpetExperiencedA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayTrumpetExperiencedB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayTrumpetExperiencedC"
					end
				...
				elseif level > 1 then
					...
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayTrumpetBeginnerA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayTrumpetBeginnerA"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayTrumpetBeginnerA"
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Trumpet"
                local randomtrumpet = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomtrumpet.iconTexture = getTexture('media/ui/trumpet_icon.png')
			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_Trumpet"
				else
					contextMenu = "ContextMenu_Play_Master_Trumpet"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/trumpet_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_Trumpet"
		...
		contextMenu = "ContextMenu_Play_Beginner_Trumpet"
		...
		contextMenu = "ContextMenu_Play_Experienced_Trumpet"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_Trumpet"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_Trumpet"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_Trumpet"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_Trumpet"
		...
		local PlayTrumpetTracksDuet = require("TimedActions/PlayTrumpetTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayTrumpetTracksDuet) do
			if v.level <= level then
...
--		local TrainingTrumpetTracks = require("TimedActions/TrainingTrumpetTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingTrumpetTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_Trumpet"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif player:getPrimaryHandItem():getFullType() == 'Base.Trumpet' then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Trumpet"
	local TrumpetWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	TrumpetWorldOption.iconTexture = getTexture('media/ui/trumpet_icon.png')
...
local function TrumpetOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if HotbarInstrument:getFullType() == 'Base.Trumpet' and
	...
	if level > 3 then
		...
		for k,v in pairs(PlayTrumpetTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Trumpet"
                local TrumpetrandomOption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				TrumpetrandomOption.iconTexture = getTexture('media/ui/trumpet_icon.png')
...
Events.OnFillInventoryObjectContextMenu.Add(TrumpetOnHotbar)