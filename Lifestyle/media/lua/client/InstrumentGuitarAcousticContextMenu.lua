...
local Instrument = { -- GUITARACOUSTIC
...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.GuitarAcoustic' then
				
			
			
				local PlayGuitarAcousticTracks = require("TimedActions/PlayGuitarAcousticTracks")
				...
				for k,v in pairs(PlayGuitarAcousticTracks) do
					if level > 3 then
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarAcoustic"
                local randomguitaroption = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitaroption.iconTexture = getTexture('media/ui/guitaracoustic_icon.png')
			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_GuitarAcoustic"
				else
					contextMenu = "ContextMenu_Play_Master_GuitarAcoustic"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/guitaracoustic_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_GuitarAcoustic"
		...
		contextMenu = "ContextMenu_Play_Beginner_GuitarAcoustic"
		...
		contextMenu = "ContextMenu_Play_Experienced_GuitarAcoustic"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_GuitarAcoustic"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_GuitarAcoustic"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_GuitarAcoustic"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_GuitarAcoustic"
		...
		local PlayGuitarAcousticTracksDuet = require("TimedActions/PlayGuitarAcousticTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayGuitarAcousticTracksDuet) do
			if v.level <= level then
...
--		local TrainingGuitarAcousticTracks = require("TimedActions/TrainingGuitarAcousticTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingGuitarAcousticTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_GuitarAcoustic"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
			elseif player:getPrimaryHandItem():getFullType() == 'Base.GuitarAcoustic' then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarAcoustic"
	local GuitarAcousticWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	GuitarAcousticWorldOption.iconTexture = getTexture('media/ui/guitaracoustic_icon.png')
...
local function GuitarAcousticOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if HotbarInstrument:getFullType() == 'Base.GuitarAcoustic' and
	...
	--3 ADDING GROUPS SUBMENU
		...
		for k,v in pairs(PlayGuitarAcousticTracks) do
			...
			--3E ADDING THE OPTION FOR EXPERT SONGS IN THE CONTEXT MENU
				contextMenu = v.name
				local subMenuEOption = subMenu:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, v.sound, length, v.level, false, false, xp, boredomReduction, stressReduction, actionType);
				if v.isaddon ~= 0 then
				subMenuEOption.iconTexture = getTexture('media/ui/addon_icon.png')
				end
			end
	...
	if level > 3 then
		...
		for k,v in pairs(PlayGuitarAcousticTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarAcoustic"
                local randomguitaroption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitaroption.iconTexture = getTexture('media/ui/guitaracoustic_icon.png')
...
break;
		...
		end--If GuitarAcoustic
	end

end

Events.OnFillInventoryObjectContextMenu.Add(GuitarAcousticOnHotbar)