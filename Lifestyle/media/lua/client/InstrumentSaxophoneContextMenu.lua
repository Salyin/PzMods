...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.Saxophone' then -- CHANGE THIS LINE
				
			
			
				local PlaySaxophoneTracks = require("TimedActions/PlaySaxophoneTracks")
				...
				for k,v in pairs(PlaySaxophoneTracks) do
					if level > 3 then
				...
				if level < 4 then
				actionType = "Bob_PlaySaxophoneDefaultBeginner"
				else
				actionType = "Bob_PlaySaxophoneDefault"
				end
			...
			if not isSitOnGround then
				...
				if randomTrack.level >= 8 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlaySaxophoneProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlaySaxophoneProficientB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlaySaxophoneProficientC"
					end
				...
				elseif randomTrack.level >= 4 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlaySaxophoneProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlaySaxophoneExperiencedC"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlaySaxophoneExperiencedC"
					end
				...
				elseif randomTrack.level >= 2 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlaySaxophoneExperiencedA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlaySaxophoneExperiencedB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlaySaxophoneExperiencedC"
					end
				...
				elseif level > 1 then
					...
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlaySaxophoneBeginnerA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlaySaxophoneBeginnerA"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlaySaxophoneBeginnerA"
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Saxophone"
                local saxophonerandomoption = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				saxophonerandomoption.iconTexture = getTexture('media/ui/saxophone_icon.png')

			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_Saxophone"
				else
					contextMenu = "ContextMenu_Play_Master_Saxophone"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/saxophone_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_Saxophone"
		...
		contextMenu = "ContextMenu_Play_Beginner_Saxophone"
		...
		contextMenu = "ContextMenu_Play_Experienced_Saxophone"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_Saxophone"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_Saxophone"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_Saxophone"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_Saxophone"
		...
		local PlaySaxophoneTracksDuet = require("TimedActions/PlaySaxophoneTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlaySaxophoneTracksDuet) do
			if v.level <= level then
...
--		local TrainingSaxophoneTracks = require("TimedActions/TrainingSaxophoneTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingSaxophoneTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_Saxophone"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif player:getPrimaryHandItem():getFullType() == 'Base.Saxophone' then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Saxophone"
	local SaxophoneWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	SaxophoneWorldOption.iconTexture = getTexture('media/ui/saxophone_icon.png')
...
local function SaxophoneOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if HotbarInstrument:getFullType() == 'Base.Saxophone' and
	...
	if level > 3 then
		...
		for k,v in pairs(PlaySaxophoneTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Saxophone"
                local saxophonerandomoption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				saxophonerandomoption.iconTexture = getTexture('media/ui/saxophone_icon.png')	
...
Events.OnFillInventoryObjectContextMenu.Add(SaxophoneOnHotbar)