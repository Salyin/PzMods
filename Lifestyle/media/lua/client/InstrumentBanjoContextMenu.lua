...
Events.OnPreFillInventoryObjectContextMenu.Add(function(player, context, items)
	...
	--SETTING BASE Stress and Boredom
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.Banjo' then --> IF BANJO <--
				
			
			
				local PlayBanjoTracks = require("TimedActions/PlayBanjoTracks")
				local AvailableTracks = {}
	
				-- SKILLED PLAYERS WON'T RANDOMLY PLAY BEGINNER SONGS, ELSE SELECT TRACKS UP TO PLAYER LEVEL
				for k,v in pairs(PlayBanjoTracks) do
					if level > 3 then
				...
				local randomNumber = ZombRand(#AvailableTracks) + 1 -- WE ADD 1 BECAUSE RANDOM STARTS AT 0
				local randomTrack = AvailableTracks[randomNumber]
				local length = randomTrack.length * 48 -- MULTIPLY BY 48 TO GET CORRECT LENGTH IN SECONDS
				
				-- DEFAULT ANIM TO PLAY WHEN SITTING
				
				if level < 4 then
				actionType = "Bob_PlayGuitarDefaultBeginner"
				else
				actionType = "Bob_PlayGuitarDefault"
				end
			...
			if not isSitOnGround then
				...
				if randomTrack.level >= 8 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarProficientB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarProficientC"
					end
				...
				elseif randomTrack.level >= 4 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarExperiencedC"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarExperiencedC"
					end
				...
				elseif randomTrack.level >= 2 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarExperiencedA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarExperiencedB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarExperiencedC"
					end
				...
				elseif level > 1 then
					...
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarBeginnerA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarBeginnerA"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarBeginnerA"
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Banjo"
                local randombanjo = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType); --> Create a new option on the context menu
				randombanjo.iconTexture = getTexture('media/ui/banjo_icon.png')
			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_Banjo"
				else
					contextMenu = "ContextMenu_Play_Master_Banjo"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/banjo_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_Banjo"
		...
		contextMenu = "ContextMenu_Play_Beginner_Banjo"
		...
		contextMenu = "ContextMenu_Play_Experienced_Banjo"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_Banjo"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_Banjo"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_Banjo"
	...
	--3F ADDING DUETS GROUP SUBMENU >3 ------------------------------- PAY ATTENTION TO THIS PART
		contextMenu = "ContextMenu_Play_Duet_Banjo"
		...
		--local subsubMenuGA = parentMenu:getNew(parentMenu);
		--context:addSubMenu(duetMenu, subMenuF)
		...
		local PlayBanjoTracksDuet = require("TimedActions/PlayBanjoTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayBanjoTracksDuet) do
			if v.level <= level then
			...
			--3F ADDING THE OPTION FOR DUETS SONGS IN THE CONTEXT MENU
				contextMenu = v.name
				...
				local subsubMenuGA = subsubMenu:addOption(getText("ItemName_Base.GuitarAcoustic"))
...
--		local TrainingBanjoTracks = require("TimedActions/TrainingBanjoTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingBanjoTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_Banjo"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif player:getPrimaryHandItem():getFullType() == 'Base.Banjo' then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Banjo"
	local BanjoWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	BanjoWorldOption.iconTexture = getTexture('media/ui/banjo_icon.png')
...
local function BanjoOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if HotbarInstrument:getFullType() == 'Base.Banjo' and
	...
	if level > 3 then
		...
		for k,v in pairs(PlayBanjoTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Banjo"
                local randombanjo = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randombanjo.iconTexture = getTexture('media/ui/banjo_icon.png')
...
Events.OnFillInventoryObjectContextMenu.Add(BanjoOnHotbar)