...
local Instrument = { -- GUITARELECTRIC
...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.GuitarElectricBlack' or
			'Base.GuitarElectricBlue' or
			'Base.GuitarElectricRed'
			then -- CHANGE THIS LINE
				
			
			
				local PlayGuitarElectricTracks = require("TimedActions/PlayGuitarElectricTracks")
				...
				for k,v in pairs(PlayGuitarElectricTracks) do
					if level > 3 then
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectric"
                local randomguitarelectricoption = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitarelectricoption.iconTexture = getTexture('media/ui/guitarelectric_icon.png')

			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_GuitarElectric"
				else
					contextMenu = "ContextMenu_Play_Master_GuitarElectric"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/guitarelectric_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_GuitarElectric"
		...
		contextMenu = "ContextMenu_Play_Beginner_GuitarElectric"
		...
		contextMenu = "ContextMenu_Play_Experienced_GuitarElectric"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_GuitarElectric"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_GuitarElectric"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_GuitarElectric"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_GuitarElectric"
		...
		local PlayGuitarElectricTracksDuet = require("TimedActions/PlayGuitarElectricTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayGuitarElectricTracksDuet) do
			if v.level <= level then
...
--		local TrainingGuitarElectricTracks = require("TimedActions/TrainingGuitarElectricTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingGuitarElectricTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_GuitarElectric"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif (player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricBlack') or
			(player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricBlue') or
			(player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricRed')
			then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectric"
	local GuitarElectricWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	GuitarElectricWorldOption.iconTexture = getTexture('media/ui/guitarelectric_icon.png')
...
local function GuitarElectricOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if (HotbarInstrument:getFullType() == 'Base.GuitarElectricBlack' or HotbarInstrument:getFullType() == 'Base.GuitarElectricBlue' or HotbarInstrument:getFullType() == 'Base.GuitarElectricRed') and
	...
	if level > 3 then
		...
		for k,v in pairs(PlayGuitarElectricTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectric"
                local randomguitarelectricoption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitarelectricoption.iconTexture = getTexture('media/ui/guitarelectric_icon.png')
...
break;
		...
		end--If GuitarElectric
	end

end

Events.OnFillInventoryObjectContextMenu.Add(GuitarElectricOnHotbar)