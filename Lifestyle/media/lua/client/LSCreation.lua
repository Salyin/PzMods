	...
	if player ~= nil then
		...
		elseif TotalPartyPoints > 0 then
			...
			if not player:HasTrait("ToneDeaf") then
				...
				elseif (playerData.LSMoodles["DJAudience"].Value == 0.4) and (OtherPlayersAround >= 8) then
				...
				elseif (playerData.LSMoodles["DJAudience"].Value == 0.2) and (OtherPlayersAround >= 5) then
					LSMoodleManager.setValue("DJAudience", 0.4)
				elseif playerData.LSMoodles["DJAudience"].Value == 0.2 and OtherPlayersAround < 3 then
					LSMoodleManager.setValue("DJAudience", 0.0)
				elseif playerData.LSMoodles["DJAudience"].Value == 0.0 and OtherPlayersAround >= 3 then
					LSMoodleManager.setValue("DJAudience", 0.2)
				end
			end
		PartyCalc = true
...
function LSEveryMinute()
	...
	if thisPlayer ~= nil then
		...
		if playerData.LSMoodles ~= nil then--MOODLES

			if playerData.LSMoodles["Embarrassed"].Value >= 0.4 then--EMBARRASSED
				stats:setStress(currentStress + 0.03);
				bodyDamage:setUnhappynessLevel(currentUnhappiness + 1)
			...
			elseif playerData.LSMoodles["Embarrassed"].Value ~= nil and playerData.LSMoodles["Embarrassed"].Value >= 0.2 then
				stats:setStress(currentStress + 0.015);
				bodyDamage:setUnhappynessLevel(currentUnhappiness + 1)
