...
function onDancingPartner(worldobjects, playerObj, DanceTargetID, DanceProposer)
...
function onDancing(worldobjects, playerObj)
    --playerObj:getModData().IsDancingFull = true
	PlayerIsDancingToMusic = require("TimedActions/PlayerIsDancingToMusic")
	local player = playerObj
	local actionType = "Bob_PreDancingDefault"
...
Events.OnFillWorldObjectContextMenu.Add(function(player, context, worldobjects)
	...
	((playerObj:getModData().IsListeningToJukebox == true) or (playerObj:getModData().IsListeningToDJ == true)) and
            ...
            if #playersList > 0 and playerObj:getModData().LSMoodles["Embarrassed"].Value ~= nil and playerObj:getModData().LSMoodles["Embarrassed"].Value < 0.2 and currentEndurance > 0.3 and debugremoveOption == false then
                for i,v in ipairs(playersList) do
					...
					v:isOutside() == playerObj:isOutside() then
						...
						local PlayerIsDancingToMusicOption = context:addOption(getText("ContextMenu_Dancing_Option"), worldobjects, onDancing, playerObj);
						local PlayerIsDancingTogetherToMusicOption = context:addOption(getText("ContextMenu_Dancing_Partner_Option", DanceTargetName.." "..DanceTargetSurname), worldobjects, onDancingPartner, playerObj, DanceTargetID, DanceProposer);
