...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.Flute' then -- CHANGE THIS LINE
				
			
			
				local PlayFluteTracks = require("TimedActions/PlayFluteTracks")
				...
				for k,v in pairs(PlayFluteTracks) do
					if level > 3 then
				...
				if level < 4 then
				actionType = "Bob_PlayFluteDefaultBeginner"
				else
				actionType = "Bob_PlayFluteDefault"
				end
			...
			if not isSitOnGround then
				...
				if randomTrack.level >= 8 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayFluteProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayFluteProficientB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayFluteProficientC"
					end
				...
				elseif randomTrack.level >= 4 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayFluteProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayFluteExperiencedC"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayFluteExperiencedC"
					end
				...
				elseif randomTrack.level >= 2 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayFluteExperiencedA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayFluteExperiencedB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayFluteExperiencedC"
					end
				...
				elseif level > 1 then
					...
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayFluteBeginnerA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayFluteBeginnerA"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayFluteBeginnerA"
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Flute"
                local randomfluteoption = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomfluteoption.iconTexture = getTexture('media/ui/flute_icon.png')
			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_Flute"
				else
					contextMenu = "ContextMenu_Play_Master_Flute"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/flute_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_Flute"
		...
		contextMenu = "ContextMenu_Play_Beginner_Flute"
		...
		contextMenu = "ContextMenu_Play_Experienced_Flute"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_Flute"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_Flute"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_Flute"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_Flute"
		...
		local PlayFluteTracksDuet = require("TimedActions/PlayFluteTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayFluteTracksDuet) do
			if v.level <= level then
...
--		local TrainingFluteTracks = require("TimedActions/TrainingFluteTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingFluteTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_Flute"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif player:getPrimaryHandItem():getFullType() == 'Base.Flute' then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Flute"
		local FluteWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
		FluteWorldOption.iconTexture = getTexture('media/ui/flute_icon.png')
...
local function FluteOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if HotbarInstrument:getFullType() == 'Base.Flute' and
	...
	if level > 3 then
		...
		for k,v in pairs(PlayFluteTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_Flute"
                local FluterandomOption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				FluterandomOption.iconTexture = getTexture('media/ui/flute_icon.png')
...
Events.OnFillInventoryObjectContextMenu.Add(FluteOnHotbar)