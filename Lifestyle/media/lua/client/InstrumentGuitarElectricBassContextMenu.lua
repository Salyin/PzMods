...
local Instrument = { -- GUITARELECTRICBASS
...
-- we loop the table in case there are more items, we can specify which item we want or apply to all items
    ...
    for i = 1, #items do
		...
		if Instrument[item:getFullType()] then
            ...
            if 'Base.GuitarElectricBassBlack' or
			'Base.GuitarElectricBassBlue' or
			'Base.GuitarElectricBassRed'
			then -- CHANGE THIS LINE
				
			
			
				local PlayGuitarElectricBassTracks = require("TimedActions/PlayGuitarElectricBassTracks")
				...
				for k,v in pairs(PlayGuitarElectricBassTracks) do
					if level > 3 then
				...
				if level < 4 then
				actionType = "Bob_PlayGuitarBassDefaultBeginner"
				else
				actionType = "Bob_PlayGuitarBassDefault"
				end
			...
			if not isSitOnGround then
				...
				if randomTrack.level >= 8 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarBassProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarBassProficientB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarBassProficientC"
					end
				...
				elseif randomTrack.level >= 4 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarBassProficientA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarBassExperiencedC"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarBassExperiencedC"
					end
				...
				elseif randomTrack.level >= 2 then
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarBassExperiencedA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarBassExperiencedB"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarBassExperiencedC"
					end
				...
				elseif level > 1 then
					...
					if randomNumberAnim >= 64 then
						actionType = "Bob_PlayGuitarBassBeginnerA"
					elseif randomNumberAnim >= 34 then
						actionType = "Bob_PlayGuitarBassBeginnerA"
					elseif randomNumberAnim >= 1 then
						actionType = "Bob_PlayGuitarBassBeginnerA"
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectricBass"
                local randomguitarelectricbassoption = context:addOption(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitarelectricbassoption.iconTexture = getTexture('media/ui/guitarelectricbass_icon.png')

			--2 ADDING THE SUBMENU TO PLAY SONGS YOUR LEVEL BY NAME
	
				if level < 10 then
					contextMenu = "ContextMenu_Play_L" .. level .. "_GuitarElectricBass"
				else
					contextMenu = "ContextMenu_Play_Master_GuitarElectricBass"
				end
	
				local buildOption = context:addOption(getText(contextMenu));
				buildOption.iconTexture = getTexture('media/ui/guitarelectricbass_icon.png')
	...
	--3 ADDING GROUPS SUBMENU
		contextMenu = "ContextMenu_Play_Group_GuitarElectricBass"
		...
		contextMenu = "ContextMenu_Play_Beginner_GuitarElectricBass"
		...
		contextMenu = "ContextMenu_Play_Experienced_GuitarElectricBass"
		...
		if level > 4 then
			...
			contextMenu = "ContextMenu_Play_Intermediate_GuitarElectricBass"
		...
		if level > 6 then
			...
			contextMenu = "ContextMenu_Play_Proficient_GuitarElectricBass"
		...
		if level > 8 then
			...
			contextMenu = "ContextMenu_Play_Advanced_GuitarElectricBass"
	...
	if level > 3 then
		...
		contextMenu = "ContextMenu_Play_Duet_GuitarElectricBass"
		...
		local PlayGuitarElectricBassTracksDuet = require("TimedActions/PlayGuitarElectricBassTracksDuet")
		local AvailableTracksDuet = {}
		for k,v in pairs(PlayGuitarElectricBassTracksDuet) do
			if v.level <= level then
			...
			--3F ADDING THE OPTION FOR EXPERT SONGS IN THE CONTEXT MENU
				...
				subsubMenuF.iconTexture = getTexture('media/ui/trumpet_icon.png')
				...
				subsubMenuT.iconTexture = getTexture('media/ui/flute_icon.png')
...
--		local TrainingGuitarElectricBassTracks = require("TimedActions/TrainingGuitarElectricBassTracks")
--		local trainingTracks = {}
--		
--		for k,v in pairs(TrainingGuitarElectricBassTracks) do
--			if v.level == level then
...
--		
--		contextMenu = "ContextMenu_Training_GuitarElectricBass"
--		
...
-- level, base xp, base mood and other defs
	...
	local StressPerSecond = 0.001
			...
			return
            elseif (player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricBassBlack') or
			(player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricBassBlue') or
			(player:getPrimaryHandItem():getFullType() == 'Base.GuitarElectricBassRed')
			then -- CHANGE THIS LINE
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectricBass"
	local GuitarElectricBassWorldOption = context:addOption(getText(contextMenu), worldobjects, OnInstrumentEquippedMenu.Play, player, equippedInstrument, randomTrack.sound, length, randomTrack.level, xp, boredomReduction, stressReduction, actionType);
	GuitarElectricBassWorldOption.iconTexture = getTexture('media/ui/guitarelectricbass_icon.png')
...
local function GuitarElectricBassOnHotbar(player, context, items)
...
-- level, base xp, base mood and other defs
	...
	for i,v in ipairs(items) do
		...
		if (HotbarInstrument:getFullType() == 'Base.GuitarElectricBassBlack' or HotbarInstrument:getFullType() == 'Base.GuitarElectricBassBlue' or HotbarInstrument:getFullType() == 'Base.GuitarElectricBassRed') and
	...
	if level > 3 then
		...
		for k,v in pairs(PlayGuitarElectricBassTracksDuet) do
			...
			-- ADDING THE OPTION TO PLAY RANDOM
				contextMenu = "ContextMenu_Play_Random_GuitarElectricBass"
                local randomguitarelectricbassoption = context:addOptionOnTop(getText(contextMenu), player, OnInstrumentMenu.Play, item, randomTrack.sound, length, randomTrack.level, false, false, xp, boredomReduction, stressReduction, actionType);
				randomguitarelectricbassoption.iconTexture = getTexture('media/ui/guitarelectricbass_icon.png')
...
break;
		...
		end--If GuitarElectricBass
	end

end

Events.OnFillInventoryObjectContextMenu.Add(GuitarElectricBassOnHotbar)