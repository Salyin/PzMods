...
function RecoverableLevels:Update(player)
	...
	if level < 10 then		
		...
		if currentXP > 0 then --make sure we only save XP if there's XP to save or we might get oddities in the math
			level = 0.0 + level + currentXP / xpForNextLevel --level with fraction based on xp over the minimum
			if level ~= level then --nan check
				level = 0
				print("UKO: Got NaN for skill \""..perk:getName().."!")
			end
		end
...
function RecoverableLevels:Recover(player)
    ...
    local affectedPerks = ""
    print("UKO: Looping through skills to recover..")
    for perkName, targetLevel in pairs(self.Content) do
	print("UKO: Recovering "..perkName..", at "..targetLevel.."..")
        local perk = PerkFactory.getPerkFromName(perkName);
	
	local config = getDataForSkill(skillConfig, perk:getId())
